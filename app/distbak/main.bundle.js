...
/***/ (function(module, exports) {
	...
	app.run(function ($rootScope, $state, $stateParams, $ionicPopup, $ionicHistory, pointData, ls) {
	    ...
	    if(!pointData.collection.isConnected){
	        pointData.connect();
	    }
...
/***/ (function(module, exports, __webpack_require__) {
	...
	app.config(["$stateProvider", "$urlRouterProvider", function($stateProvider, $urlRouterProvider) {
	    ...
	    $stateProvider
	        ...
	        // 购买积分
	        .state("buyPoint", {
	            templateUrl: "./donation/buyPoint/buyPoint.html",
	            url: "/buyPoint",
	            controller: "buyPointController",
	            cache: false
	        })
	        ...
	        .state("main.integralTrading", {
	            ...
	            views: {
	                'main-integralTrading': {
	                    ...
	                    controller: "integralTradingMainController",
	                }
	            },
	            cache: false
	        })
...
/***/ (function(module, exports, __webpack_require__) {
	...
	app.controller("walletListController",function($scope, $state, $rootScope, $http, $location, pointData) {
...
/***/ (function(module, exports, __webpack_require__) {
	...
	app.controller("buyPointController",function($scope, $state, $rootScope, $http, $location, pointData, $ionicLoading) {
		$scope.data = pointData.collection;
	    ...
	    $http({
			...
			if(ret.success) {
	            $ionicLoading.hide();
				$scope.baseFree = ret.data.baseFree;
	            $scope.vm.price = pointData.collection.market.curPrice;
	            price = pointData.collection.market.curPrice;
	            $scope.calc();
			} else {
				$scope.alert(ret.msg);
			}
		...
		$scope.calc = function(){
	        ...
	        $scope.maxBuyNumber = (Math.floor(($scope.baseFree/($scope.vm.price * (1+pointData.collection.market.buyFeeRatio)))/10))*10;
		};
...
/***/ (function(module, exports, __webpack_require__) {
	...
	app.controller("orderController", function($scope, $rootScope, $http, $location,$timeout, $state, $ionicTabsDelegate, $ionicSlideBoxDelegate, $ionicScrollDelegate, $ionicModal) {
	    ...
	    $scope.sign = function(order,$event){
...
/***/ (function(module, exports, __webpack_require__) {
	...
	app.controller("orderSubmitController", function($scope, $rootScope, $http, $location, $state, $ionicLoading, $ionicModal, $ionicHistory, ls) {
	    ...
	    $scope.getData = function(){
	        var _orderLoded = false;
	        var _addressLoded = false;
	        $ionicLoading.show({ template: '数据加载中...' });
	        $http({
	            ...
	            _orderLoded = true;
	            if(_addressLoded){$ionicLoading.hide();}
	        ...
	        $http({
	            ...
	            _addressLoded = true;
	            if(_orderLoded){$ionicLoading.hide();}
	        }).error(function(ret){
	            $ionicLoading.hide();
	            $scope.alert('数据加载失败');
	        });
	    ...
	    $scope.updateProductList = function(data){
	        ...
	        $scope.cutPointFree = data.scoreProxy.cutPointFree;
	        localStorage.setItem('productList', JSON.stringify($scope.productList));
	    }
...
/***/ (function(module, exports, __webpack_require__) {
	...
	app.controller("orderPayController", function($scope, $rootScope, $http, $window, $timeout, $stateParams, $state, $ionicLoading, $ionicModal, $ionicHistory, $ionicPopup) {
	    ...
	    $http({
	        ...
	        $scope.cutPointFree = ret.data.scoreProxy.cutPointFree;
	        // 推广钱包
	        $scope.userWalletRewardLeft = ret.data.scoreProxy.oldWalletReward - ret.data.scoreProxy.cutGrowFree;
	        $scope.userWalletRewardLost = ret.data.scoreProxy.cutGrowFree;
	        // 用户积分 用户消费券
	        $scope.userScore = ret.data.scoreProxy.oldPointFree;
	        $scope.userScoreTicket = ret.data.scoreProxy.oldPoint2Free;
	    ...
	    $scope.paySuccess = function(ret){
	        ...
	        $scope.cutGrowFree = ret.data.cutGrowFree;//推广钱包
	        $scope.cutPointFree = ret.data.cutPointFree;//实付交易积分
	        $scope.cutPoint2Free = ret.data.cutPoint2Free;//实付消费积分
...
/***/ (function(module, exports, __webpack_require__) {
	...
	app.controller("accountController", function($scope, $rootScope, $http, $location, $state, $stateParams, $ionicLoading) {
	    ...
	    $http({
	        ...
	        $ionicLoading.hide();
	        var data = ret.data;
	        $scope.bankNum = data.bankNum;
	        $scope.depositBank = data.depositBank;
	        $scope.detailBank = data.detailBank;

	        $scope.bankNum2 = data.bankNum2;
	        $scope.depositBank2 = data.depositBank2;
	        $scope.detailBank2 = data.detailBank2;

	        $scope.alipayId = data.alipayId;
	    }).error(function(ret) {
	        $ionicLoading.hide();
	    });

	    $scope.update = function(){
	        $state.go("addAccount",{ bankNum: $scope.bankNum, depositBank: $scope.depositBank, detailBank: $scope.detailBank, bankNum2: $scope.bankNum2, depositBank2: $scope.depositBank2, detailBank2: $scope.detailBank2});
	    };
...
/***/ (function(module, exports, __webpack_require__) {
	...
	app.factory('pointData', function($rootScope, $websocket, $http, $interval) {
	    ...
	    var msgHandler = function(message) {
	        ...
	        if (data.msg === "market") {
	            ...
	            collection.wallet.pointTotal = collection.wallet.point + collection.wallet.point2;
	            collection.wallet.walletTotal = collection.wallet.wallet + collection.wallet.wallet2;
	            collection.wallet.cost = (collection.wallet.pointAvg * collection.wallet.pointTotal).toFixed(2);
	        }else if(data.msg === "heartbeat"){
	            _heartBeat = new Date();
	            collection.isOpen = data.data;
	        }
	        ...
	        collection.todayChanges = Number(((collection.market.curPrice - collection.market.closePrice) * (collection.wallet.point+collection.wallet.point2)).toFixed(2)) + Number(collection.wallet.profit);
	    ...
	    var methods = {
	        ...
	        ws: function(){
	            ...
	            var url = 'ws://106.14.206.109:9999/ws?a=' + localStorage.getItem("token");
	            // var url = 'ws://192.168.1.6:8080/ws?a=' + localStorage.getItem("token");
	            dataStream = $websocket(url);
	            dataStream.onMessage(msgHandler);
	            dataStream.onClose(function(closeEvent){
	                collection.isConnected = false;
	            });
	            dataStream.onOpenCallbacks.push(function(){
	                _heartBeat = new Date();
	                collection.isConnected = true;
	            });
	        },
	        connect: function() {
	            var that = this;
	        ...
	        link: function(scope, element, attr) {
	            ...
	            if (attr.source) {
	                scope.$watch(attr['source'], function() {
	                    ...
	                    var date = new Date();
	                    var timeLength = data.length?data[data.length-1][0] - data[0][0]:0;
	                    var startPercent = timeLength?(100-(3600000/timeLength)*100).toFixed(2):0;
	                    myChart.setOption({
	                        dataZoom:[{
	                            type: 'inside',
	                            start: startPercent,
	...
	app.controller("integralTradingMainController", function($scope, pointData, $ionicPopup) {
	    $scope.data = pointData.collection;
	...
	app.controller("integralTradingController", function($scope, $rootScope, $http, $state, $stateParams, pointData, $timeout) {
	    $scope.data = pointData.collection;
	    ...
	    if (!pointData.is_K_pushed()) {
	        $http({
	            ...
	            for (var i in ret.data) {
	                pointData.collection.groupData.push([new Date((ret.data[i].createTime).replace(/-/g,'/')), ret.data[i].curPrice]);
	            }
	            pointData.k_pushed();
	        }).error(function(ret) {
	        });
...
/***/ (function(module, exports, __webpack_require__) {
	...
	app.controller("dealController", function ($scope, $rootScope, $http, $ionicLoading, pointData) {
	    $scope.tab_index = 0;
	    ...
	    $scope.getData = function(){
	        var _waitLoded = false;
	        var _completeLoded = false;
	        $ionicLoading.show({ template: '数据加载中...' });
	        $http({
	            ...
	            for(var i in $scope.waitList){
	                ...
	                $scope.waitList[i].total = ($scope.waitList[i].price * $scope.waitList[i].amount).toFixed(2);
	                $scope.waitList[i].tradeTotal = ($scope.waitList[i].tradePrice * $scope.waitList[i].tradeAmount).toFixed(2);
	            }
	            _waitLoded = true;
	            if(_completeLoded){$ionicLoading.hide();}
	        }).error(function(ret){
	            $ionicLoading.hide();
	            $scope.alert('数据加载失败');
	        });
	        ...
	        $http({
	            ...
	            for(var i in $scope.completeList){
	                ...
	                $scope.completeList[i].total = ($scope.completeList[i].price * $scope.completeList[i].amount).toFixed(2);
	            }
	            _completeLoded = true;
	            if(_waitLoded){$ionicLoading.hide();}
	        }).error(function(ret){
	            $ionicLoading.hide();
	            $scope.alert('数据加载失败');
	        });
...
/***/ (function(module, exports, __webpack_require__) {
	...
	app.controller("managementController", function ($scope, $state, $http, pointData, $ionicModal) {
	    $scope.data = pointData.collection;

	    $scope.transferIn = function () {
	        $ionicModal.fromTemplateUrl('./integralTrading/management/transferIn.html', {
	            scope: $scope,
	            animation: 'slide-in-up'
	        }).then(function (modal) {
	            $scope.transferInModal = modal;
	            $scope.transferInModal.show();
	        });
	    };

	    $scope.closetransferInModal = function(){
	        $scope.transferInModal.hide();
	    };

	    $scope.$on('$destroy', function () {
	        if($scope.transferInModal){$scope.transferInModal.remove();}
	    });

	});

	app.controller("transferInController", function ($scope, $state, $http, $ionicLoading){
	    var imgFile;
	    $scope.vm={
	        'amount': 0
	    }
		...
		$scope.submit= function(){
			...
			$http({
				...
				$ionicLoading.hide();
				if(ret.success) {
	                $scope.alert('上传打款截图成功');
	                $scope.closetransferInModal();
				}
			}).error(function(ret){
				$ionicLoading.hide();
				$scope.alert(ret);
...
/***/ (function(module, exports, __webpack_require__) {
	...
	app.controller("fundRollOutController", function($scope, $state, $rootScope, $http, $location, $stateParams, $ionicLoading) {
	    $scope.vm = {
	        'amount': 0,
	        'cardId': 1
	    };
	    ...
	    $http({
	        ...
	        $scope.bankNum = ret.data.bankNum;
	        $scope.depositBank = ret.data.depositBank;
	        $scope.detailBank = ret.data.detailBank;

	        $scope.bankNum2 = ret.data.bankNum2;
	        $scope.depositBank2 = ret.data.depositBank2;
	        $scope.detailBank2 = ret.data.detailBank2;
	    }).error(function(ret) {
	    });
	    ...
	    $scope.submit = function(){
	        $ionicLoading.show({ template: '数据加载中...' });
	        $http({
	            url: ROOTCONFIG.host + "/p/exit?amount=" + $scope.vm.amount + "&card=" + $scope.vm.cardId,
	            method: "post",
	        }).success(function(ret) {
	            $ionicLoading.hide();
				if(ret.success) {
...
/***/ (function(module, exports, __webpack_require__) {
	...
	app.controller("tradeFlowController", function($scope, $state, $rootScope, $http, $ionicScrollDelegate, $ionicLoading) {
	    ...
	    $http({
	        ...
	        list = ret.data;
	        for(var i in list){
	            list[i].time = new Date((list[i].createTime).replace(/-/g,'/'));
	            list[i].total = (list[i].price * list[i].amount).toFixed(2);
	            if(list[i].isBuy==1){
	                listBuy.push(list[i]);
	            }else{
	                listSell.push(list[i]);
	            }
	        }
	        $scope.list = list;
	    }).error(function(ret) {
