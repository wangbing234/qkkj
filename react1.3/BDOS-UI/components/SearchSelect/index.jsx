...
const SearchSelect = React.createClass({
  ...
  handleClick(item) {
    ...
    this.state.result.length > 0 ? this.setState({selected: true, disabled: true}) : this.setState({
      selected: false,
      disabled: false
    });
        ...
        <DropdownToggle>
          {
            this.state.selected ?
              (<div>
                <ul className="form-control s-s-u">{_result}</ul>
              </div> ) :
              (<input onChange={this.handleChange} onFocus={this.handleFocus} className="form-control"
