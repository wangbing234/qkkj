...
class BFDChart extends React.Component {
  constructor(prop) {
    ...
    this.state = {
      ...
      isLoad: false,  //是否加载报图数据结束,true 则展示报图
      isDateRange: prop.isDateRange,  //时间选择器类型, 范围, 还是单个时间
      dayCols: {dtcount: '访问次数'},
      hourCols: {hrcount: '访问次数', elapsedms: '平均响应时长(ms)'},
      data: prop.data || [],
      remark: prop.isDateRange ? '近一星期访问数' : '今天访问情况',
      title: prop.isDateRange ? '近一周API访问情况' : 'API日访问情况',
      ...prop.data
    };
  }

  componentDidMount() {
    console.log("render BFDChart")
  ...
  getLineChart(startTime, endTime) {
    if (that.state.dataType === 0) { //获取总访问量
      Model.getAPIDayStatistics({
        startDt: startTime,
        endDt: endTime,
        api: that.state.idValue
      }, function (result) {
  ...
  renderLineChart() {
    //有数据才渲染报图
    if (that.state.isLoad && that.state.data && that.state.data.length > 0) {
  ...
  renderDateTime() {
    if (that.state.isDateRange) {
      return (<DateRange start={that.state.startTime}
                         end={that.state.endTime} style={{marginLeft:20}} onSelect={that.handleSelect}/>)
    } else {
  ...
  render() {
    ...
    return (
      <div className="module-container">
        ...
        <h5 style={{marginLeft:20,marginBottom:35}}>API URL: <a style={{marginLeft:10}}>{that.state.idValue}</a></h5>
