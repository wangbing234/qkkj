...
import AddMySqlAPI from './AddMySqlAPI'
...
class ResoureConfig extends React.Component {
   ...
   */
  create() {
    that.setState({isAddPage: true, ResourceConfigItem: null})
  }
  ...
  del(item) {
    ...
    if (ids) {
      confirm('确定删除 MySql API?', () => {
        Model.del(ids, function (result) {
          message.success("删除 MySql API 成功!")
   ...
   */
  renderAddUser() {
    return (<AddMySqlAPI parent={that} data={that.state.ResourceConfigItem}/>)
  }
