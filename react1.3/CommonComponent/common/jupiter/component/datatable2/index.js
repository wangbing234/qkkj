 ...
 * 添加自动翻页的功能, 设置 autoPage = true, 即可自动翻页  by zhongxia
 */
...
class Rows extends Component {
  ...
  render() {
    ...
    let subRows = this.props.rows;

    if (this.props.autoPage) {
      subRows = rows.slice((currentPage - 1) * this.props.pageSize, currentPage * this.props.pageSize);
    }
    return (
      <tbody>
      {
        subRows.length > 0 ?
          subRows.map((item, j) => {
            ...
            const checkboxTd = this.props.onCheckboxSelect ?
              (<td><Checkbox disabled={isDisabled} checked={isSelect}
                             onClick={this.handleCheckboxClick.bind(this)}
                             onChange={this.handleCheckboxChange.bind(this, item)}></Checkbox></td>)
              : null
            return (
          ...
          <td colSpan="99">
            <div className="align-center" ref="nothingData"></div>
            暂无数据!
          </td>
        </tr>
      }
...
class DataTable extends Component {
  ...
  render() {
    ...
    const checkboxTh = this.props.onCheckboxSelect ?
      <th><Checkbox checked={this.state.isSelectAll} onChange={this.handleCheckboxAllChange.bind(this)}></Checkbox>
      </th> : null
    return (
      <div>
        {url != '' ? <Fetch url={url} onSuccess={this.handleSuccess.bind(this)}></Fetch> : null}
        ...
        <table className={classnames('bfd-datatable', 'bfd-table', className)} {...other} >
              ...
              column.map((head, i) => {
                ...
                return (
                  <th
                    ...
                    title={head['order']===true ? head['title'] + '排序' : ''}
                    className={head['order']===true ? 'bfd-datatable--sorting' : ''}>
                    {head['title']}
                  </th>
                )
              })
          ...
          <Rows
            ...
            onSelect={this.handleCheckboxChange.bind(this)}
            onCheckboxSelect={this.props.onCheckboxSelect&&this.props.onCheckboxSelect.bind(this)}
            column={this.props.column}
            autoPage={this.props.autoPage}
