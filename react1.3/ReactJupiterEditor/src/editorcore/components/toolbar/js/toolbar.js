export default (function () {
   ...
   */
  function hideOrShow() {
    $('.dialog-group-title').on('click', function (e) {
      const $tar = $(e.currentTarget)
      const $list = $tar.siblings('.dialog-group-content')
      if ($tar.attr('data-show') === '1') {
        $list.slideUp(200)
        $tar.attr('data-show', '0')
        $tar.find('.arrow').css('transform', 'rotate(0deg)')
      } else {
        $list.slideDown(200)
        $tar.attr('data-show', '1')
        $tar.find('.arrow').css('transform', 'rotate(180deg)')
      }
    })
  }
  ...
  function dragNode2Paper(id, callbackDrop) {
    const elePaper = document.getElementById(id),
