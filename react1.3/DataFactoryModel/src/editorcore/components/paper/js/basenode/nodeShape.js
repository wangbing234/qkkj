...
joint.shapes.html = {
  ...
  ElementView: joint.dia.ElementView.extend({
    ...
    updateBox: function () {
      ...
      var _iconType;
      if(_data && _data.iconType!=1 && (_iconType=_data.iconType))
      {
        if(_iconType==2) {//其它项目表
          this.$box.find('.logo').attr('src', "src/common/img/editor/logo/other.png");
        }
        else  if(_iconType==3){//共享项目表
          this.$box.find('.logo').attr('src', "src/common/img/editor/logo/share.png");
        }
        else{
          this.$box.find('.logo').hide();
        }
      }
      else {
        this.$box.find('.logo').hide();
