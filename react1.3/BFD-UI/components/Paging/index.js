        ...
        <Row>
          ...
          <div className="bfd-paging__layout-right">
            <ul className="bfd-paging__pagination">
              <li>
                <a onClick={this.handleLaquoClick.bind(this)} className={'bfd-paging__pagination-li--prev '+ (currentIndex === 1 ? 'bfd-paging__pagination-li--frist' : '')}>上一页</a>
              </li>
              {this.getPages(currentIndex, pageNum)}
              <li>
                <a onClick={this.handleRaquoClick.bind(this)} className={'bfd-paging__pagination-li--next '+ (currentIndex === pageNum ? 'bfd-paging__pagination-li--end' : '')}>下一页</a>
              </li>
            </ul>
            {!this.props.hideGo ? (
                <div className="bfd-paging__go">
                  <label className="bfd-paging__label-font">跳转到：</label>
                  <Input onChange={this.checkNumber.bind(this)} value={this.state.goPageNum} className="bfd-paging__go-number"/>
                  <Button onClick={this.handleGoPage.bind(this)} className="btn btn-primary">GO</Button>
                </div>
            ) : ''}
          </div>
        </Row>
  ...
  getPages(currentPage, maxPage){
    ...
    if(maxPage <= showPage){
      for(let i = 1; i <= maxPage; i++) {
        let html = this.createPageEl(i,pages.length+1);
        pages.push(html);
      }
    }else{
      //当总页数小于或等于需要显示的页码（加上第一页与最后一页）时，直接添加进数组
      if(maxPage <= showPage + 1) {
        for(let i = 1; i <= maxPage; i++) {
          let html = this.createPageEl(i,pages.length+1)
          pages.push(html)
        }
        ...
        if(currentPage < showPage){
          ...
          pages.push(<li key={pages.length+1}><span>...</span></li>);
          pages.push(this.createPageEl(maxPage,pages.length+1));
        }else if(currentPage < maxPage){//前面带...
          pages.push(this.createPageEl(1,pages.length+1));
          pages.push(<li key={pages.length+1}><span>...</span></li>);
          pages.push(this.createPageEl(currentPage-1,pages.length+1));
          pages.push(this.createPageEl(currentPage,pages.length+1));
          pages.push(this.createPageEl(currentPage+1,pages.length+1));
          if(currentPage+2 == maxPage){
            pages.push(this.createPageEl(maxPage,pages.length+1));
          }else if(currentPage + 1 != maxPage){//后面也带...
            pages.push(<li key={pages.length+1}><span>...</span></li>);
            pages.push(this.createPageEl(maxPage,pages.length+1));
          }
        }else{//后面带...
          pages.push(this.createPageEl(1));
          pages.push(<li key={pages.length+1}><span>...</span></li>);
          pages.push(this.createPageEl(currentPage-1,pages.length+1));
          pages.push(this.createPageEl(maxPage,pages.length+1));
        }
