...
export default React.createClass({
  ...
  handleUserInput(filterText) {
    let arr = this.filterSource(filterText);
    this.setState({
      filterText,
      searchData: arr
    })
  },
  filterSource(filterText){
    const arr = []
  ...
  componentWillReceiveProps(nextProps) {
    if(nextProps.sdata != this.props.sdata) {
      let searchValue = this.refs.searchBar.searchValue;
      let sArr = searchValue?this.filterSource(searchValue):nextProps.sdata;
      this.setState({
    ...
    return (
      <div className="bfd-transfer">
        <div className="col-xs-4">
          <SearchBar  ref="searchBar" onUserInput={this.handleUserInput}/>
          <SourceTable 
