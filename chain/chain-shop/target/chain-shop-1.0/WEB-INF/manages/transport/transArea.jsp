...
//disable已经选择的区域信息
   ...
   jQuery(".area_box_main>ul>li").each(function(){
      var count=jQuery(this).find(".area_level :checkbox[checked=true]").length;
	  if(count>0){
   ...
   jQuery(":checkbox[id^=province_]").click(function(){
	  ...
	  var count=jQuery(this).parent().parent().parent().parent().find(":checkbox[id^=province_][checked=true]").length;
   ...
   jQuery(":checkbox[id^=city_]").click(function(){
      var count=jQuery(this).parent().parent().parent().parent().parent().find(":checkbox[id^=city_][checked=true]").length;
	  ...
	  var p_count=jQuery(this).parent().parent().parent().parent().parent().parent().parent().parent().find(":checkbox[id^=province_][checked=true]").length;
...
function generic_area(){
  ...
  var citys="";
  var city_ids="";
  jQuery(":checkbox[id^=city_][checked=true]").each(function(){
...
<div class="area_box">
	<div class="area_box_title">
		<span class="area_box_title_left">选择区域</span> <span
	...
	<c:forEach items="${objs}" var="obj">
		...
		<div class="area_bg_white">
			<div class="area_before">
				<input name="group_${obj.id}" id="group_${obj.id}" type="checkbox"
					value="${obj.id}" /><label style="padding-left: 3px;"
					for="group_${obj.id}">${obj.areaName}</label>
			</div>
			<div class="area_box_main">
				<ul>

					<c:forEach items="${obj.childs}" var="info">

						<li><input name="province_${info.id}"
							id="province_${info.id}" type="checkbox" value="${info.id }" />
							<span><label for="province_${info.id}"
								style="float: left;">${info.areaName}</label><b
								id="city_count_${info.id}" style="color: #F60"></b></span> <c:if
								test="${not empty info.childs }">

								<div class="area_level">
									<ul>


										<c:forEach items="${info.childs}" var="info1">

											<li><label><input name="city_${info1.id}"
													id="city_${info1.id}" city_name="${info1.areaName}"
													type="checkbox" value="${info1.id}" />${info1.areaName}</label></li>
										</c:forEach>
										<li class="close"><input type="button" value="关闭"
											onclick="jQuery(this).parent().parent().parent().hide();jQuery('.area_box_main li').removeClass('this');" /></li>
									</ul>
								</div>
							</c:if></li>
					</c:forEach>
