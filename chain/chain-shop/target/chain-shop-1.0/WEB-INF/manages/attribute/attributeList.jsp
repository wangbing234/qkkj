...
<body>
	<s:form action="attribute" namespace="/manage" method="post"
		theme="simple">
		<s:hidden name="e.pid" />
		<table class="table table-bordered">
			<tr>
				<td style="text-align: right;">商品分类</td>
				<td><input type="hidden" id="catalogID2"
					value="<s:property value="e.catalogID"/>" /> <%
			...
			<tr>
				<td colspan="2">
					...
					<button method="attribute!selectList.action"
					...
					<s:a method="toAdd" cssClass="btn btn-success">
						...
						<i class="icon-plus-sign icon-white"></i> 添加
							</s:a> <%-- 							<s:submit method="deletes" onclick="return deleteSelect();" value="删除" cssClass="btn btn-danger" /> --%>

					<%-- 							<s:a method="deletes" cssClass="btn btn-danger" onclick="deleteSelect();"> --%>
					<!-- 								<i class="icon-remove-sign icon-white"></i> 删除 --> <%-- 							</s:a> --%>
					<button method="attribute!deletes.action" class="btn btn-danger"
						onclick="return submitIDs(this,'确定删除选择的记录?');">
						<i class="icon-remove-sign icon-white"></i> 删除
					</button> <s:if test="e.pid==0">
					...
					<div
						...
						<%@ include file="/WEB-INF/manages/system/attrPager.jsp"%>
					</div>
				</td>
			</tr>
		</table>
			...
			<s:iterator value="pager.list">
				<tr>
					...
					<td nowrap="nowrap"><s:a
							href="attribute!toEdit.action?e.id=%{id}">编辑</s:a></td>
				</tr>
			</s:iterator>

			<tr>
				<td colspan="7" style="text-align: center;"><%@ include
						file="/WEB-INF/manages/system/attrPager.jsp"%></td>
			</tr>
		</table>
