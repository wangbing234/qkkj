...
<body>
	...
	<s:form action="attribute" namespace="/manage" theme="simple" id="form">
		...
		<table class="table table-bordered">
			<tr style="background-color: #dff0d8">
				<td colspan="2"
					style="background-color: #dff0d8; text-align: center;"><strong>属性编辑</strong>
					<s:if test="e.pid==0">
			...
			<tr>
				...
				<td style="text-align: left;">
					...
					data-options="url:'<%=request.getContextPath() %>/manages/catalog/catalog!getRootWithTreegrid.action?e.type=p',method:'get',required:false" 
					>
				(注意：只能选择子目录。)
					 --> <%
					application.setAttribute("catalogs", SystemManager.catalogs);
					%> <select onchange="catalogChange(this)" name="e.catalogID"
					id="catalogSelect" data-rule="required;select;">
						<option></option>
			...
			<tr>
				...
				<td style="text-align: left;"><s:textfield name="e.name"
						data-rule="required;name;length[1~20];" id="name" /></td>
			</tr>
			<tr>
			...
			<tr style="background-color: #dff0d8">
				<td><input type="button" onclick="addAttribute();" value="增加属性"
			...
			<tr id="cloneTR" style="display: none;">
				<td class="leftTD_css">名称</td>
				<td style="text-align: left;"><s:textfield name="e.attrNames"
						placeholder="请输入属性名称" /> <span style="margin-left: 150px;"></span>
					显示顺序：<s:textfield number="y" name="e.order1List"
						placeholder="请输入属性显示的顺序" value="0" size="5" maxlength="5" /></td>
			</tr>
				...
				<s:iterator value="e.attrList" var="item" status="i">
					<s:if test="#i.index==0">
						...
						<tr class="pclass">
							...
							<td style="text-align: left;"><s:textfield
									name="e.attrNames0" id="attrNames0" value="%{name}"
									data-rule="required;attrNames0;length[1~20];" /> <s:textfield
									name="e.idList0" id="idList0" value="%{id}"
									data-rule="required;idList0;length[1~20];"
									cssStyle="display:none;" /> <span style="margin-left: 150px;"></span>
								显示顺序：<s:textfield number="y" name="e.order1List0"
									id="order1List0" value="%{order1}" size="5" maxlength="5"
									data-rule="required;integer;order1List0;length[1~5];" /></td>
					...
					<s:else>
						<tr class="pclass">
							<td class="leftTD_css">名称</td>
							<td style="text-align: left;"><s:textfield
									name="e.attrNames" value="%{name}" /> <s:textfield
									name="e.idList" value="%{id}" cssStyle="display:none;" /> <span
								style="margin-left: 150px;"></span> 显示顺序：<s:textfield number="y"
									name="e.order1List" id="order1List" value="%{order1}" size="5"
									maxlength="5"
									data-rule="required;integer;order1List;length[1~5];" /> <span
								style="margin-left: 150px;"><a href="javascript:void(0)"
									onclick="del(this,${id})">删除</a></span></td>
						</tr>
					</s:else>
				</s:iterator>
			</s:if>
			<s:else>
				<tr class="pclass">
					<td class="leftTD_css">名称</td>
					<td style="text-align: left;"><s:textfield name="e.attrNames0"
							id="attrNames0" data-rule="required;attrNames0;length[1~20];"
							placeholder="请输入属性名称" /> <span style="margin-left: 150px;"></span>
						显示顺序：<s:textfield number="y" name="e.order1List0" id="order1List0"
							size="5" maxlength="5"
							data-rule="required;order1List0;integer;length[1~5];"
							placeholder="请输入属性显示的顺序" /></td>
				</tr>
			</s:else>
		</table>
	</s:form>
	...
	$(function(){
		...
		window.location.href="manage/attribute!deletes.action?id=${e.id}&ids="+id;
		}}
