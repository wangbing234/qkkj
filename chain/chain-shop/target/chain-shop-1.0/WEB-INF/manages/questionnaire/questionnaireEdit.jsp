...
<body>
	<s:form action="questionnaire" namespace="/manage" theme="simple"
		id="form">

		<div
			style="text-align: center; border-bottom: 1px solid #ccc; padding: 5px;">
			<s:if test="e.id=='' or e.id==null">
				<s:submit method="insert" onclick="return onSubmit();" value="新增问卷"
					cssClass="btn btn-primary" />
			</s:if>
			<s:else>
				<a class="btn btn-warning"
					href="<%=SystemManager.systemSetting.getWww() %>/question/show.html?id=<s:property value="e.id" />"
					target="_blank">查看</a>
				<s:submit method="update" onclick="return onSubmit();" value="保存问卷"
					cssClass="btn btn-primary" />

				<s:a
					href="questionnaire!toEdit.action?e.id=%{e.id}&e.addQuestionnaireItem=true&e.editQuestionnaireItem=false"
					cssClass="btn btn-success">增加题目</s:a>
				<!-- 		<input type="button" id="addQuestionItemBtn" value="增加题目" onclick="return addQuestionItemFunc(this);" class="btn btn-warning"/> -->
				<s:submit method="deleteItem" onclick="return deleteSelect();"
					value="删除题目" cssClass="btn btn-warning" />
			</s:else>
		...
		<table class="table">
			<tr>
				...
				<td style="text-align: left;"><s:hidden
						name="e.questionnaireItem.id"></s:hidden> 题目：<s:textfield
						name="e.questionnaireItem.subject" id="subject"
						placeholder="请输入问卷题目内容" /> <s:select
						list="#{'radio':'单选','checkbox':'多选','text':'文本'}"
						name="e.questionnaireItem.type" /> <s:select
						list="#{'inline':'一行显示','lines':'多行显示'}"
						name="e.questionnaireItem.display" /> 顺序<s:textfield
						name="e.questionnaireItem.order1" id="order1"
						placeholder="请输入题目顺序,必须是数字" /> <br> <!-- 编辑题目模式 --> <s:if
						test="e.editQuestionnaireItem">
						<s:iterator value="e.questionnaireItem.optionList" status="i"
							var="item">
						选项：<input type="text" name="e.questionnaireItem.optionArr"
								id="option" width="400px" maxLength="100"
								value="<s:property value="item" />" />
							<br>
						</s:iterator>
					</s:if> 选项：<s:textfield name="e.questionnaireItem.optionArr" id="option"
						placeholder="请输入选项内容" width="400px" maxLength="100" /><br>
					选项：<s:textfield name="e.questionnaireItem.optionArr" id="option"
						placeholder="请输入选项内容" width="400px" maxLength="100" /><br>
					选项：<s:textfield name="e.questionnaireItem.optionArr" id="option"
						placeholder="请输入选项内容" width="400px" maxLength="100" /><br>
					选项：<s:textfield name="e.questionnaireItem.optionArr" id="option"
						placeholder="请输入选项内容" width="400px" maxLength="100" /> <!-- 				<input type="button" value="添加选项" class="btn"> -->
					<br></td>
			</tr>
			<tr>
				<td colspan="2" style="text-align: center;"><s:if
						test="e.questionnaireItem.id=='' or e.questionnaireItem.id==null">
						<s:submit method="insertQuestionItem" action="questionnaire"
							onclick="return onSubmit(this);" value="新增题目"
							cssClass="btn btn-warning" />
					</s:if> <s:else>
						<s:submit method="insertQuestionItem" action="questionnaire"
							onclick="return onSubmit(this);" value="修改题目"
							cssClass="btn btn-warning" />
					</s:else> <input id="cancelAddQuestionItemBtn" type="button" value="取消"
					class="btn" /></td>
			</tr>
		</table>
		</div>
			...
			<div id="tabs-1">
				<table class="table table-condensed">
					...
					<tr>
						...
						<td style="text-align: left;"><s:textarea name="e.title2"
								rows="3" cols="40" cssStyle="width:500px;" id="questionnaire"></s:textarea></td>
					</tr>
					<tr>
						<td style="text-align: right;">是否启用</td>
						<td style="text-align: left;"><s:select
								list="#{'n':'否','y':'是'}" id="status" name="e.status"
			...
			<div id="tabs-2">
				<table class="table table-condensed">
					<tr style="background-color: #dff0d8">
						<th width="20" nowrap="nowrap"><input type="checkbox"
							id="firstCheckbox" /></th>
						<th>序号</th>
						<th>题目</th>
						<th>类型</th>
					...
					<s:iterator value="e.questionnaireItemMap" status="i" var="item">
						<tr>
							...
							<td>&nbsp; <s:a
									href="questionnaire!toEdit.action?e.id=%{value.qid}&e.questionnaireItem.id=%{value.id}&e.editQuestionnaireItem=true">
									<s:property value="value.subject" />
								</s:a><br>
								...
								<div style="margin-left: 20px;">
									<div style="display: table-row;">
										...
										<s:else>
											<s:iterator value="value.optionList" status="ii" var="option">
												<s:property value="#ii.index+1" />、<s:property
													value="option" />
												<br>
											</s:iterator>
										</s:else>
							...
							<td width="60px"><s:if test="value.type.equals(\"radio\")">单选</s:if>
								<s:elseif test="value.type.equals(\"checkbox\")">多选</s:elseif> <s:else>文本</s:else>
							</td>
							<td><s:textfield name="value.order1" /></td>
						</tr>
					</s:iterator>
				</table>
			</div>
