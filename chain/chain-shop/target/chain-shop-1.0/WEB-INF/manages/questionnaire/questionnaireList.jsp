		...
		<table class="table table-bordered">
			<tr>
				...
				<td style="text-align: left;"><s:textfield name="e.title"
						cssClass="search-query input-small" id="title" /></td>
				<td style="text-align: right;">状态</td>
				<td style="text-align: left;"><s:select
						list="#{'':'','y':'已启用','n':'已禁用'}" id="status" name="e.status"
						cssClass="input-medium" listKey="key" listValue="value" /></td>
				<td style="text-align: right;">创建时间</td>
				<td style="text-align: left;" colspan="3"><input id="d4311"
			...
			<tr>
				<td colspan="16"><s:submit method="selectList" value="查询"
						cssClass="btn btn-primary"></s:submit> <s:submit method="toAdd"
						action="questionnaire" value="添加" cssClass="btn btn-success" /> <s:submit
						method="deletes" onclick="return deleteSelect('确定删除选择的记录?');"
						value="删除" cssClass="btn btn-danger" /> <%-- 							<s:submit method="updateInBlackList" onclick="return updateInBlackList();" value="加入黑名单" cssClass="btn btn-warning" /> --%>
					<s:submit method="setStatusY" value="显示"
						onclick="return deleteSelect('确定让选择的记录审核通过?这样选择的记录将会出现在门户上。');"
						cssClass="btn btn-warning" /> <s:submit method="setStatusN"
						value="不显示" cssClass="btn btn-warning"
						onclick="return deleteSelect('执行该操作后,选择的记录将不会出现在门户上。确定要执行?');" />
		...
		<table class="table table-bordered table-hover">
			...
			<s:iterator value="pager.list">
				<tr>
					...
					<td>&nbsp; <s:a href="questionnaire!toEdit.action?e.id=%{id}">
							<s:property value="title" />
						</s:a>
					</td>
					<td>&nbsp;<s:property value="createdate" /></td>
					<td>&nbsp; <s:a
							href="user!toEdit.action?e.username=%{createAccount}">
							<s:property value="createAccount" />
						</s:a>
					</td>
					<td>&nbsp;<s:property value="updatedate" /></td>
					<td>&nbsp; <s:a
							href="user!toEdit.action?e.username=%{updateAccount}">
					...
					<td>&nbsp;<s:property value="lastLoginIp" /> <s:property
							value="showStartDate" />~<s:property value="showEndDate" />
					</td>
					<td><s:a href="questionnaire!toEdit.action?e.id=%{id}">编辑</s:a>|
						<a
						href="<%=SystemManager.systemSetting.getWww() %>/question/show.html?id=<s:property value="id" />"
