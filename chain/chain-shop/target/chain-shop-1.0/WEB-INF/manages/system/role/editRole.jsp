...
<link rel="stylesheet"
	...
	type="text/css">
<link rel="stylesheet"
	href="<%=request.getContextPath()%>/resource/bootstrap/css/docs.css"
	type="text/css">
...
<%-- <%@ include file="/WEB-INF/manage/system/common.jsp"%> --%>
...
<SCRIPT type="text/javascript">
		$(function(){
			...
			function loadMenusTree(id){
				$.ajax({
					url:"<%=request.getContextPath()%>/manage/menu!getMenusByPid.action?pid=0&id="+id,
					type:"get",
					dataType:"text",
		...
		<table class="table table-bordered"
			...
			<tr>
				...
				<td style="text-align: left;"><s:if test="role.id==null">
						<s:textfield name="role.role_name" id="role_name" readonly="false" />
					</s:if> <s:else>
			...
			<tr>
				...
				<td style="text-align: left;"><s:select
						list="#{'select':'select','select,insert':'select,insert','select,insert,update':'select,insert,update','select,insert,update,delete':'select,insert,update,delete'}"
						name="role.role_dbPrivilege" id="role_dbPrivilege" /></td>
			</tr>
			<tr>
