		...
		<table class="table table-bordered table-condensed">
			<tr style="display: none">
				<td style="text-align: right;">角色名称</td>
			  	<td><s:textfield cssClass="input-small" name="role.role_name" style="width:105px;" />&nbsp;&nbsp;&nbsp;</td>
				<td>状态</td>
				<td><s:select list="#{'':'全部','y':'启用','n':'禁用'}" id="status" name="role.status" style="width:120px;" cssClass="input-small" listKey="key" listValue="value" /></td>
			</tr>
			<tr>
				<td colspan="4">
				 ...
				 <%} %> <%if(PrivilegeUtil.check(request.getSession(), "role!insert.action")){%>
					...
					<s:a method="toAdd" cssClass="btn btn-success">
						<i class="icon-plus-sign icon-white"></i> 添加
							</s:a> <%} %> <%if(PrivilegeUtil.check(request.getSession(), "role!deletes.action")){%>
		...
		<table class="table table-bordered table-hover">
			...
			<s:iterator value="pager.list">
				<tr>
					...
					<td>
						<!-- 系统角色只能是超级管理员编辑 --> <%-- 							<s:if test="id==1"> --%> <%
									User user = (User)request.getSession().getAttribute(ManageContainer.manage_session_user_info);
									if(user.getUsername().equals("admin")){
								%> <s:a href="role!toEdit.action?id=%{id}">编辑</s:a> <%} %> <%-- 							</s:if> --%>
						<%-- 							<s:else> --%> <%-- 								<s:a href="role!toEdit.action?id=%{id}">编辑</s:a> --%>
						<%-- 							</s:else> --%>
					</td>
				</tr>
			</s:iterator>
			<tr>
				<td colspan="15" style="text-align: center;"><%@ include
