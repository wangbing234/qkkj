...
<SCRIPT type="text/javascript">
	...
	$(function(){
 		$("#add").add("#update").click(function(){
 			art.dialog.open('/manage/menu!toEdit.action',
		...
		<tr class="warning">
			<td colspan="2">
				<div>
					...
					提示：点击菜单项，此处则能编辑该菜单项或增加顶级菜单或子菜单项。<br> <input type="button"
						id="deleteMenus" value="删除选择的菜单" class="btn btn-danger" /> <a
						id="initAreaTree"
						href="<%=request.getContextPath()%>/manage/area!initAreaTree.action"
						class="btn btn-danger" onclick="return initAreaTreeFunc();">初始化省市区数据</a>
					<font color="red">(默认只删除叶子菜单)</font> [<a
						id="expandOrCollapseAllBtn" href="#" title="展开/折叠全部资源"
						onclick="return false;">展开/折叠</a>]
				</div>
			</td>
		</tr>
		<tr>
			<td>
				<div style="min-width: 200px;">
					<div id="loadImg" style="text-align: center;">
						<img alt="菜单加载中......"
							src="<%=request.getContextPath() %>/resource/images/loader.gif">资源加载中...
					</div>
					<ul id="areaTree" style="display: none;" class="ztree"></ul>
				</div>
			</td>
			<td><iframe src="" width="600" id="iframeMenuEdit" height="800">
					点击菜单项，此处则能编辑该菜单项或增加顶级菜单或子菜单项。 </iframe></td>
		</tr>
	</table>
...
var setting = {
	...
	callback: {
		...
		enable: true,
		type: "post",
		url: "/manage/area!getAreaTree.action",
		autoParam: ["code=pid"]
