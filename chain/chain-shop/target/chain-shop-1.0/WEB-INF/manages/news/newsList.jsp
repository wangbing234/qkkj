...
<body>
	<s:form action="news" namespace="/manage" method="post" theme="simple"
		id="form" name="form">
		...
		<table class="table table-bordered">
			<tr>
				<td>标题</td>
				<td><s:textfield cssClass="input-medium search-query"
						name="e.title" /></td>
				<s:if test="e.type.equals(\"help\")">
					<td>文章类型</td>
					<td>
						<%-- 							<input id="cc" class="easyui-combotree" name="e.catalogID" value="<s:property value="e.catalogID"/>" --%>
						<%-- 							data-options="url:'<%=request.getContextPath() %>/manages/catalog/catalog!getRootWithTreegrid.action?type=a',method:'get',required:false"  --%>
						<!-- 							> --> <%
				...
				<td><s:select cssStyle="width:100px;"
						list="#{'':'','y':'显示','n':'不显示'}" name="e.status" id="status"
			...
			<tr>
				<td colspan="16">
					...
					<%-- 						<s:param name="btnSelect" value="1">1</s:param> --%> <!-- 						<i class="icon-search icon-white"></i> 查询 -->
					<%-- 					</s:submit> --%> <%-- 					<s:a method="selectList" cssClass="btn btn-primary"> --%>
					<!-- 						<i class="icon-search icon-white"></i> 查询 --> <%-- 					</s:a> --%>
					<button method="news!selectList.action" class="btn btn-primary"
						onclick="selectList(this)">
						<i class="icon-search icon-white"></i> 查询
					</button> <%-- 					<s:submit method="toAdd" action="news" value="添加" cssClass="btn btn-success" /> --%>
					<s:a method="toAdd" cssClass="btn btn-success">
						<s:param name="type" value="e.type"></s:param>
						<i class="icon-plus-sign icon-white"></i> 添加
					</s:a> <%-- 					<s:submit method="deletes" onclick="return deleteSelect('确定删除选择的记录?');" value="删除" cssClass="btn btn-danger" /> --%>
					<button method="news!deletes.action" class="btn btn-danger"
						onclick="return submitIDs(this,'确定删除选择的记录?');">
						<i class="icon-remove-sign icon-white"></i> 删除
					</button> <%-- 					<s:submit method="updateStatusY" value="显示" onclick="return deleteSelect('确定让选择的记录审核通过?这样选择的记录将会出现在门户上。');" cssClass="btn btn-warning" /> --%>
					<button method="news!updateStatusY.action" class="btn btn-warning"
						onclick="return submitIDs(this,'确定让选择的记录审核通过?这样选择的记录将会出现在门户上。');">
						<i class="icon-arrow-up icon-white"></i> 显示
					</button> <%-- 					<s:submit method="updateStatusN" value="不显示" cssClass="btn btn-warning" onclick="return deleteSelect('执行该操作后,选择的记录将不会出现在门户上。确定要执行?');"/> --%>
					<button method="news!updateStatusN.action" class="btn btn-warning"
						onclick="return submitIDs(this,'执行该操作后,选择的记录将不会出现在门户上。确定要执行?');">
						<i class="icon-arrow-down icon-white"></i> 不显示
					</button>

					<div
						style="float: right; vertical-align: middle; bottom: 0px; top: 10px;">
						<%@ include file="/WEB-INF/manages/system/newsPager.jsp"%>
					</div>
				</td>
			</tr>
		</table>
		...
		<table class="table table-bordered table-hover">
			...
			<s:iterator value="pager.list">
				<tr>
					...
					<td class="aCss"><s:a href="news!toEdit.action?e.id=%{id}">
					...
					<td><s:a href="news!toEdit.action?e.id=%{id}">编辑</s:a>| <s:if
							test="type.equals(\"help\")">
							<a target="_blank"
								href="<%=SystemManager.systemSetting.getWww()%>/help/<s:property value="code" />.html">查看</a>
						</s:if> <s:else>
							<a target="_blank"
								href="<%=SystemManager.systemSetting.getWww()%>/news/<s:property value="id" />.html">查看</a>
						</s:else></td>
				</tr>
			</s:iterator>
			...
			<tr>
				<td colspan="17" style="text-align: center; font-size: 12px;"><%@ include
						file="/WEB-INF/manages/system/newsPager.jsp"%></td>
			</tr>
		</table>
		...
		<div class="alert alert-info"
			...
			图标含义：<BR> <img alt="显示"
				src="<%=request.getContextPath() %>/resource/images/action_check.gif">：显示到门户上
			<img alt="不显示"
				src="<%=request.getContextPath() %>/resource/images/action_delete.gif">：不显示到门户上
		</div>
