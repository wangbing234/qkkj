...
<body>
	<div class="alert alert-info">
		提示：对【商品促销】的添加/修改不会立即生效，需要到系统管理--缓存管理页面点击【加载活动+活动商品列表】按钮，才能生效。</div>

	<s:form action="activity" namespace="/manage" theme="simple" id="form"
		cssClass="form-horizontal">
		<table class="table table-bordered">
			<tr>
				<td colspan="2"
					style="background-color: #dff0d8; text-align: center;"><strong>活动编辑</strong>
				</td>
			</tr>
			...
			<tr>
				...
				<td style="text-align: left;"><s:textfield name="e.name"
						id="name" data-rule="活动名称:required;name;length[1~45];" /></td>
			</tr>
			<tr>
				...
				%> <%-- 				<s:checkboxlist cssClass="checkbox-inline" cssStyle="text-align: left;display: inline;"  --%>
					...
					<input type="checkbox" name="e.accountRange" value="R1"
					id="e.accountRange-1" data-rule="checked"> <label
					for="e.accountRange-1" class="checkboxLabel">普通会员</label> <input
					type="checkbox" name="e.accountRange" value="R2"
					id="e.accountRange-2"> <label for="e.accountRange-2"
					class="checkboxLabel">铜牌会员</label> <input type="checkbox"
					name="e.accountRange" value="R3" id="e.accountRange-3"> <label
					for="e.accountRange-3" class="checkboxLabel">银牌会员</label> <input
					type="checkbox" name="e.accountRange" value="R4"
					id="e.accountRange-4"> <label for="e.accountRange-4"
					class="checkboxLabel">金牌会员</label> <input type="checkbox"
					name="e.accountRange" value="R5" id="e.accountRange-5"> <label
					for="e.accountRange-5" class="checkboxLabel">钻石会员</label> <input
					type="hidden" id="__multiselect_form_e_accountRange"
					name="__multiselect_e.accountRange"
					value="<s:property value="e.accountRange"/>">
			...
			<tr>
				...
				<td style="text-align: left;"><input id="startDate"
					class="Wdate search-query " type="text" name="e.startDate"
					value="<s:property value="e.startDate" />"
					data-rule="活动开始时间:required;startDate;"
			...
			<tr>
				...
				<td style="text-align: left;"><input id="endDate"
					class="Wdate search-query " type="text" name="e.endDate"
					value="<s:property value="e.endDate" />"
					data-rule="活动结束时间:required;endDate;"
			...
			<tr>
				...
				<td style="text-align: left;"><s:select
						list="#{'':'','n':'禁用','y':'启用'}" data-rule="状态:required;status;"
						id="status" name="e.status" cssClass="input-small" listKey="key"
						listValue="value" /></td>
			</tr>

			<tr>
				<td style="text-align: right;">参与活动的商品</td>
				<td style="text-align: left;"><s:textfield id="productID"
						name="e.productID" cssStyle="width:80%;" /> <br>多个商品ID之间用|分割
				</td>
			</tr>
			...
			<tr>
				...
				<td style="text-align: left;"><s:select
						list="#{'':'','c':'促销活动','j':'积分兑换','t':'团购活动'}" id="activityType"
						name="e.activityType" listKey="key" listValue="value"
						data-rule="活动类型:required;activityType;" />
					<p>
					<div id="discountDiv" style="display: none;">
						<p>
							优惠方式：
							<s:select list="#{'-':'-','r':'减免','d':'折扣','s':'双倍积分'}"
								id="discountType" name="e.discountType" listKey="key"
								listValue="value" data-rule="优惠方式:required;discountType;" />
						</p>
						<p>
							折扣/减价：
							<s:textfield id="discount" name="e.discount"
					...
					<div id="exchangeScoreDiv" style="display: none;">
						兑换积分：
						<s:textfield id="exchangeScore" name="e.exchangeScore"
							cssClass="input-small" />
					</div>
					...
					<div id="minGroupCountDiv" style="display: none;">
						<p>
							最低团购人数：
							<s:textfield id="minGroupCount" name="e.minGroupCount"
								cssClass="input-small" />
						</p>
						<p>
							团购价：
							<s:textfield id="tuanPrice" name="e.tuanPrice"
								cssClass="input-small" />
						</p>
					</div></td>
			</tr>
			...
			<tr>
				...
				<td style="text-align: left;"><s:textfield id="maxSellCount"
						name="e.maxSellCount" cssClass="input-small" listKey="key"
						listValue="value"
						data-rule="最大购买数量:required;integer;maxSellCount;" /> <br>
					(0：表示不限制)</td>
			</tr>
			<tr>
