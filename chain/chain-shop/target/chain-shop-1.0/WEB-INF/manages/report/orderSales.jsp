...
<body>
	<s:form action="report!orderSales.action" namespace="/manage"
		...
		<table class="table table-bordered">
			...
			<tr>
				<td colspan="16">
					<button method="report!orderSales.action" class="btn btn-primary"
		...
		<table class="table table-bordered table-hover">
			...
			<c:forEach items="${list}" var="item">
				<tr>
					...
					<c:if test="${(e.startDate != '' && e.startDate != null) || (e.endDate != '' && e.endDate != null)}">
						<td>&nbsp;${e.startDate }</td>
						<td>&nbsp;${e.endDate }</td>
					</c:if>
					<td>&nbsp;${item.productSellCount}</td>
					<td>&nbsp;${item.sumAmount+item.walletAmount+item.scoreAmount}</td>
					<td>&nbsp;${item.walletAmount}</td>
					<td>&nbsp;${item.scoreAmount}</td>
				</tr>
			</c:forEach>
		</table>
	</s:form>
