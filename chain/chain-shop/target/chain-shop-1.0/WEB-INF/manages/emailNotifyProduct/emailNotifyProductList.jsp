...
<body>
	<s:form action="emailNotifyProduct" namespace="/manage" method="post"
		theme="simple">
		<table class="table table-bordered">
			<tr>
				...
				<td style="text-align: left;"><s:textfield
						name="e.receiveEmail" cssClass="input-small" id="receiveEmail" /></td>
			...
			<tr>
				<td colspan="28">
					...
					<button method="emailNotifyProduct!selectList.action"
						class="btn btn-primary" onclick="selectList(this)">
						<i class="icon-search icon-white"></i> 查询
					</button> <%-- 					<s:submit method="autoNotify" value="autoNotify" cssClass="btn btn-primary"/> --%>
					<button method="emailNotifyProduct!autoNotify.action"
						class="btn btn-success" onclick="submitNotValid2222(this)">
						<i class="icon-ok icon-white"></i> 触发通知
					</button> <%-- 					<s:checkbox name="systemNotify" value="true" fieldValue="Y"/> 系统自动通知 --%>
					<div
		...
		<div class="alert alert-info">发送失败3次的系统将不再尝试发送!</div>
		...
		<table class="table table-bordered table-hover">
			...
			<s:iterator value="pager.list">
				<tr>
					...
					<td nowrap="nowrap">&nbsp; <a
						href="<%=SystemManager.systemSetting.getManageHttp()%>/manages/product!toEdit.action?e.id=<s:property value="productID" />.html"
						target="_blank"> <s:property value="productID" />
					</a>
					</td>
					<td nowrap="nowrap">&nbsp; <a
						href="<%=SystemManager.systemSetting.getWww()%>/product/<s:property value="productID" />.html"
						target="_blank" title="<s:property value="productName" />"> <s:property
								value="productName" /></a>
					</td>
					<td nowrap="nowrap">&nbsp;<s:property value="createdate" /></td>
					<td nowrap="nowrap">&nbsp;<s:property value="notifydate" /></td>
					<td nowrap="nowrap" style="text-align: center;"><s:if
							test="status.equals(\"y\")">
							<img alt="显示"
