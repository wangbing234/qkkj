...
<body>
	<s:form action="accounts!selectList" namespace="/manage" method="post"
		theme="simple">
		<table class="table table-bordered">
			<tr>
				<td style="text-align: right;" nowrap="nowrap">账号</td>
				<td style="text-align: left;"><s:textfield name="e.account"
						cssClass="search-query input-small" id="account" /></td>
				<td style="text-align: right;" nowrap="nowrap">昵称</td>
				<td style="text-align: left;"><s:textfield name="e.nickname"
						cssClass="input-small" id="nickname" /></td>
				<td style="text-align: right;" nowrap="nowrap">会员类型</td>
				<td style="text-align: left;"><s:select
						list="#{'':'','R1':'普通会员','R2':'认种会员'}" id="rank" name="e.rank"
						cssClass="input-medium" listKey="key" listValue="value" /></td>
				<td style="text-align: right;" nowrap="nowrap">会员性质</td>
				<td style="text-align: left;"><s:select
						list="#{'':'','geren':'个人会员','qiye':'企业会员'}" id="qiye"
						name="e.qiye" cssClass="input-medium" listKey="key"
						listValue="value" /></td>
				<td style="text-align: right;" nowrap="nowrap">状态</td>
				<td style="text-align: left;"><s:select
						list="#{'':'','y':'已冻结','n':'未冻结'}" id="freeze" name="e.freeze"
						cssClass="input-medium" listKey="key" listValue="value" /></td>
			...
			<tr>
				<td colspan="28">
					...
					<button method="accounts!selectList.action" class="btn btn-primary"
					...
					<div
						...
						<%@ include file="/WEB-INF/manages/system/accountPager.jsp"%>
					</div>
				</td>
			</tr>
		</table>
		...
		<table class="table table-bordered table-hover">
			<tr style="background-color: #dff0d8">
				...
				<th nowrap="nowrap">昵称/公司</th>
				<th nowrap="nowrap">会员类型</th>
				<th align="center">会员性质</th>
			...
			<s:iterator value="pager.list">
				<tr>
					...
					<td nowrap="nowrap" align="center"><s:if
							test="accountType.equals(\"qq\")">
							<img alt=""
								src="<%=request.getContextPath() %>/resource/images/mini_qqLogin.png">
						</s:if> <s:elseif test="accountType.equals(\"sinawb\")">
							<img alt=""
								src="<%=request.getContextPath() %>/resource/images/mini_sinaWeibo.png">
						</s:elseif> <s:elseif test="accountType.equals(\"alipay\")">
							<span class="badge badge-warning">alipay</span>
						</s:elseif> <s:elseif test="accountType.equals(\"weixin\")">
							<span class="badge badge-warning">weixin</span>
						</s:elseif> <s:elseif test="accountType.equals(\"guest\")">
							<span class="badge badge-warning">游客登陆</span>
						</s:elseif> <s:else>
							<span class="badge badge-warning">online</span>
						</s:else></td>
					<td nowrap="nowrap">&nbsp;<s:property value="account" />
					</td>
					<td nowrap="nowrap">&nbsp;<s:property value="nickname" /></td>
					<td nowrap="nowrap">&nbsp;<s:property value="rankName" /></td>
					<td nowrap="nowrap"><c:choose>
							<c:when test="${not empty trueName }">
						企业
					...
					<td nowrap="nowrap">&nbsp; <s:if test="freeze.equals(\"y\")">
							<font color="red">冻结</font>
						</s:if> <s:elseif test="freeze.equals(\"n\")">
							正常
						</s:elseif> <s:else>
							异常
						</s:else>
					...
					<td nowrap="nowrap">
						...
						<s:a target="_blank"
							href="accounts!show.action?account=%{account}">查看</s:a> <%-- 						<s:a href="account!czmm.action?e.id=%{id}">重置密码</s:a> --%>
						| <s:if test="freeze.equals(\"y\")">
							<s:a href="accounts!toFreeze.action?e.id=%{id}">解冻</s:a>
						</s:if> <s:elseif test="freeze.equals(\"n\")">
							<s:a href="accounts!toFreeze.action?e.id=%{id}">冻结</s:a>
						</s:elseif>
			...
			<tr>
				<td colspan="16" style="text-align: center;"><%@ include
						file="/WEB-INF/manages/system/accountPager.jsp"%></td>
			</tr>
		</table>
	</s:form>
