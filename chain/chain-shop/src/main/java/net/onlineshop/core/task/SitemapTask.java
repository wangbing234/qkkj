...
public class SitemapTask implements Runnable {
	...
	private void createSiteMap() {
		...
		try {
			...
			urlsetElement.addAttribute("xmlns ", "http://www.sitemaps.org/schemas/sitemap/0.9"); // "xmlns
																									// "必须要有空格,否则会报错
			urlsetElement.addAttribute("xmlns", "http://www.sitemaps.org/schemas/sitemap/0.9");

			urlsetElement.addAttribute("xmlns:xsi", "http://www.w3.org/2001/XMLSchema-instance");

			urlsetElement.addAttribute("xsi:schemaLocation", "http://www.sitemaps.org/schemas/sitemap/0.9 " +
			...
			while (fp.nextFile()) {
				if (!fp.isDirectory()) {
					...
					if ((fname.equals(".html") || fname.equals(".htm")) || fname.equals(".jsp") && !f.equals("login.html")) {
