...
import net.onlineshop.web.action.manage.area.AreaAction;
...
public class FrontCache {
	...
	public void setAreaAction(AreaAction areaAction) {
	...
	public void loadPlugConfig() {
		...
		Pay yeePay = new Pay();
		yeePay.setCode(Pay.pay_code_weixin);
		...
		if (pay != null) {
			...
			Pay afterPay = new Pay();
			afterPay.setCode(Pay.pay_code_afterpay);
			afterPay = payService.selectOne(yeePay);
			SystemManager.payCode = afterPay.getCode();
		}
	...
	public void loadNewCatalogs() {
		...
		List<Catalog> iCatalogs = new ArrayList<Catalog>();
		...
		SystemManager.newCatalogs = iCatalogs;
	}
	...
	public static List<Attribute> loadAttrByCatalogID(int catalogID) {
		...
		if (attrs == null || attrs.size() == 0) {
			catalogID = Integer.parseInt(FrontCache.getPid(String.valueOf(catalogID)));
	 ...
	 * @throws Exception
	 */
	public void loadIndexCatalogs(boolean loadProduct) throws Exception {
		logger.debug("load...");
		...
		if (catalogsList != null) {
			SystemManager.catalogsIndex.clear();
			Map<Catalog, List<Product>> map = new LinkedHashMap<Catalog, List<Product>>();
			for (Iterator<Catalog> it = catalogsList.iterator(); it.hasNext();) {
				...
				if (StringUtils.isNotBlank(item.getPid()) && (!item.getPid().equals("0"))) {
					...
					product.setTop(8);// 显示的最大个数
					product.setCatalogID(item.getId());
					List<Product> indexProducts = productService.loadHotProductShowInIndex(product);
					if (indexProducts != null && indexProducts.size() > 0) {
						map.put(item, indexProducts);
					} else {
						map.put(item, null);
			...
			SystemManager.catalogsIndex = map;
		}
		
			SystemManager.catalogsItems.clear();
			Product product = new Product();
			product.setProductType(ProductType.DONATION.getIndex());// 显示的最大个数
			product.setTop(6);// 显示的最大个数
			product.setStatus(2);
			SystemManager.catalogsItems = productService.selectList(product);
	}
...
//			}
			...
			if("0".equals(item.getPid())){
				SystemManager.catalogsItem=item;
			}
	...
	public void loadIndexImgs() {
		...
		c.setType(0);
		List<IndexImg> list = indexImgService.selectList(c);
		SystemManager.indexImages = list;
		IndexImg c1 = new IndexImg();
		c1.setType(2);
		List<IndexImg> list1 = indexImgService.selectList(c1);

		SystemManager.indexImagesPc = list1;

		IndexImg floor1 = new IndexImg();
		floor1.setType(1);

		List<IndexImg> f1 = indexImgService.selectList(floor1);
		SystemManager.indexf1 = f1;
		IndexImg floor2 = new IndexImg();
		floor2.setType(3);

		List<IndexImg> f2 = indexImgService.selectList(floor2);
		SystemManager.indexf2 = f2;
		IndexImg floor3 = new IndexImg();
		floor3.setType(4);

		List<IndexImg> f3 = indexImgService.selectList(floor3);
		SystemManager.indexf3 = f3;
	 ...
	 * 加载九宫格图片列表
	 */
	public void loadindexNineImgs() {
		logger.debug("loadIndexImgs...");
		IndexImg c = new IndexImg();
		c.setType(1);

		SystemManager.indexNineImages = indexImgService.selectList(c);
	...
	public void readJsonArea() {
		...
		try {
			File file = new File(CoreParamCache.getInstance().get("areaJson").toString());
			SystemManager.areaMap=new HashMap<String, Area>();
			//文件不存在
			if(file.exists())
			{
				logger.debug("开始读取区域路径！"+file.getAbsolutePath());
				List<String> list = FileUtils.readLines(file, "utf-8");
				SystemManager.areaMap = JSON.parseObject(list.get(0), new TypeReference<Map<String, Area>>() {});
			}
			else{
				logger.debug("区域路径不存在！");
//				areaAction.initAreaTree();
			}
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	private void loadIndexLeftProduct() {
		...
		p1.setTop(1);
		p1.setProductType(ProductType.DONATION.getIndex());
//		Product p2 = new Product();
//		p2.setTop(1);
//		p2.setProductType(ProductType.OPEN.getIndex());
		Product p3 = new Product();
		p3.setTop(1);
		p3.setProductType(ProductType.POINT.getIndex());
		List<Product> tempIndexLeftProduct=new ArrayList<Product>();
		tempIndexLeftProduct.addAll(productService.selectPageLeftHotProducts(p1));
//		tempIndexLeftProduct.addAll(productService.selectPageLeftHotProducts(p2));
		tempIndexLeftProduct.addAll(productService.selectPageLeftHotProducts(p3));
		logger.info(SystemManager.getInstance().hashCode()+""); 
		SystemManager.indexLeftProduct=tempIndexLeftProduct;
	}
	...
	public void loadActivityScoreProductList() {
		...
		for (Iterator<Entry<String, Activity>> it = SystemManager.activityMap.entrySet().iterator(); it.hasNext();) {
			...
			if (entry.getValue().getActivityType().equals(Activity.activity_activityType_j) && StringUtils.isNotBlank(entry.getValue().getProductID())) {
	...
	public void loadActivityTuanProductList() {
		...
		for (Iterator<Entry<String, Activity>> it = SystemManager.activityMap.entrySet().iterator(); it.hasNext();) {
			Entry<String, Activity> entry = it.next();
			if (entry.getValue().getActivityType().equals(Activity.activity_activityType_t) && StringUtils.isNotBlank(entry.getValue().getProductID())) {
				String[] arr = entry.getValue().getProductID().split("\\|");
				for (int i = 0; i < arr.length; i++) {
					productIds.add(arr[i]);
				}
