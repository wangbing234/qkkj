...
public class MenuService implements Services<Menu> {
	...
	/**
	 * 根据菜单ID，删除菜单树勾选的节点。某个非叶子节点即便它下面的所有的叶子节点都被勾选也不会被本次删除操作删除掉， 这样做是为了避免只想删除某个非叶子节点下面的所有子节点
	 * 
	 * @param ids
	 * @param deleteParent
	...
	public void deletes(String ids, String deleteParent) {
		...
		if (deleteParent.equals("-1")) {
			...
			for (int i = 0; i < idArr.length; i++) {
				/*
				 * 1、菜单下没有子菜单，直接删除 2、菜单下有子菜单，检查所有的子菜单是否全部已经勾选 A)全部勾选，则可以删除。 B)没有全部勾选，则不能删除。
				 */
				menu.clear();
				menu.setPid(idArr[i]);
