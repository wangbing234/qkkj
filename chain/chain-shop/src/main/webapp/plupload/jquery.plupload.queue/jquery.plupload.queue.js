	...
	function b(f, e) {
		...
		e
				.prepend('<div class="plupload_wrapper plupload_scroll"><div id="'
						...
						+ '</div><a href="javascript:void(0);" onclick="closeUpload();" title="关闭" class="i-close">关闭</a></div></div><div class="plupload_content"><div class="plupload_filelist_header"><div class="plupload_file_name">'
						+ a("Filename")
					...
					.each(function() {
						...
						function f() {
							...
							c("span.plupload_upload_status", i).html(
									a("完成上传 %d/%d 文件&nbsp;<a href='javascript:void(0);' onclick='upload()'>重新上传</a>").replace(
											/%d\/%d/,
											j.total.uploaded + "/"
													+ j.files.length))
						}
						...
						j.bind("Error", function(l, o) {
							...
							if (m) {
								...
								if (o.code == plupload.FILE_SIZE_ERROR) {
									alert(a("对不起，您上传的文件太大,不能超过"+plupload.formatSize(l.settings.max_file_size)+": ") + m.name)
								}
								if (o.code == plupload.FILE_EXTENSION_ERROR) {
									alert(a("对不起，不支持该文件: ") + m.name)
								}
								m.hint = n;
												...
												c(
														...
														.text(
																"完成上传 "
																		...
																		+ "/"
																		+ j.files.length
																		+ " 文件&nbsp;<a href='javascript:void(0);' onclick='upload()'>重新上传</a>");
