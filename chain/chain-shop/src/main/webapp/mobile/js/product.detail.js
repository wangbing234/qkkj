...
$(function() {
	// 选项卡
	TouchSlide({
		slideCell:"#olexon-tab-box",
		endFun:function(i){
			if(i == 0) {
				
				// 产品图文详情
				var dateil = $('.olexon-prod-dateil');
				setTimeout(function() {
					$('.olexon-prod-tab .tempWrap').css({'height':'auto'});
					
				}, 100);
				if(dateil.html() == '') {
					
					dateil.html($('#olexon-prod-dateil-tpl').html());
					$('table', dateil).removeAttr('width');
					$('img', dateil).scrollLoading();
				}
		
			}else if(1 == i) {
				
		
				var url = __API__+'?a=getProductParameter&pid='+$('#productID').val()+"&p="+Math.random();
				...
				// 产品评论
				var comment = $('.olexon-prod-comment');
				if(comment.html() == '') {
					// 加载中...
					//comment.html('<div class="tc"><i class="loading"></i>加载中...</div>');
					// 模板
					var html = '<ul class="list clearfix"><li class="p10"><div class="olexon-nothing"><i class="fa fa-comments"></i><p>没有评论....</p></div></li></ul>';
					comment.html(html);
					// api
//					var url = __API__+'?c=Product&a=getComment&pid='+$('#productID').val()+"&p="+Math.random();
...
//					});
				...
				// 售后保障
				var dateil = $('.olexon-prod-server');
				if(dateil.html() == '') {
					dateil.html($('#olexon-prod-server-tpl').html());
					$('img', dateil).scrollLoading();
				}
			}
			var bd = document.getElementById("olexon-tab-box-bd");
			...
			if($("#specSize li").hasClass("on") && $("#specColor li").hasClass("on")){
				...
				for(var i=0;i<specJsonStringObject.length;i++){
					...
					if(specItem.specSize==$("#specSize .on").html() 
						...
						$("#nowPriceHidden").val(specItem.specPrice);
						$("#total0").text(specItem.specPrice);
