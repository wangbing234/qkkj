			  ...
			  success: function(data){
				...
				if(data.result==0){
					 ...
					 if(data.total=="0.00"){
						...
						var	defhtml='<div class="olexon-nothing"><i class="fa fa-shopping-cart"></i><p>购物车空空如也，赶快去购物吧....</p><p><a href="/" class="btn btn_red ">去逛逛</a></p></div>';
								 
						 $("#cartdiv").html(defhtml);
					 }
...
function notifyCart(_obj){
	var _url = "/cart/notifyCart.html?currentBuyNumber="+_obj.val()+"&productID="+_obj.attr("pid")+"&buySpecID="+_obj.attr("buySpecID")+"&radom="+Math.random();
...
function buyCart(productId,buyCount,specId,obj){
	var _url = "/cart!addToCart.action?productID="+productId+"&buyCount="+buyCount+"&buySpecID="+specId;
	$.ajax({
	  type: 'POST',
	  url: _url,
	  data: {},
	  success: function(data){
		...
		if(data==0){
			setFocusTip($(obj),"添加购物车成功");
		}else{
	
			setFocusTip($(obj),data.tips);
			resetProductInfo();
		}
	  },
	  dataType: "json",
	  error:function(e){
		  ...
		  setFocusTip($(obj),'加入购物车失败！请联系客服寻求解决办法。');
		  return false;
	  }
	});
}
...
//	}
	...
	var _url = "/cart!addToCart.action?productID="+$("#productID").val()+"&buyCount="+buy_num+"&buySpecID="+$("#specIdHidden").val();
	$.ajax({
	  type: 'POST',
	  url: _url,
	  data: {},
	...
	$.ajax({
	  ...
	  success: function(data){
		  ...
		  if(data=="0"){
			 ...
			 $(".ff").addClass("ee").removeClass("ff").text("已收藏");
...
function resetProductInfo(){
	...
	//设置值为商品原价格
	$("#nowPrice").text("¥"+$("#priceNow").val());
	$("#nowPriceHidden").val($("#priceNow").val());
	$("#total0").text($("#priceNow").val());
