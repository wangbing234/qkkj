...
<body>
	<s:form action="activity" namespace="/manage" method="post"
		theme="simple">
		<table class="table table-bordered">
			<tr>
				<td>活动ID</td>
				<td><s:textfield cssClass="input-small search-query"
						name="e.id" /></td>
				<!-- 				<td>商品ID</td> -->
				...
				<td style="text-align: left;"><s:select
						...
						name="e.activityType" cssClass="input-small" listKey="key"
						listValue="value" /></td>
				<td style="text-align: right;">优惠方式</td>
				<td style="text-align: left;"><s:select
						list="#{'':'','r':'减免','d':'折扣','s':'双倍积分'}" id="discountType"
						name="e.discountType" cssClass="input-small" listKey="key"
						listValue="value" /></td>
				<td>状态</td>
			...
			<tr>
				<td colspan="18">
					<button method="activity!selectList.action" class="btn btn-primary"
						onclick="selectList(this)">
						<i class="icon-search icon-white"></i> 查询
					</button> <s:a method="toAdd" cssClass="btn btn-success">
						<i class="icon-plus-sign icon-white"></i> 添加
					</s:a>
					<button method="activity!deletes.action" class="btn btn-danger"
			...
			<s:iterator value="pager.list">
				<tr>
					...
					<td nowrap="nowrap">&nbsp; <s:if
							test="activityType.equals(\"c\")">
							<span class="badge badge-info">促销活动</span>
						</s:if> <s:elseif test="activityType.equals(\"j\")">
							<span class="badge badge-success">积分兑换</span>
						</s:elseif> <s:elseif test="activityType.equals(\"t\")">
					...
					<td nowrap="nowrap">活动时间：<s:property value="startDate" /> ~ <s:property
							value="endDate" /> <s:if test="expire">
							<span class="label label-danger" style="background-color: Red;">活动已到期</span>
						</s:if> <br> 商品ID：<s:property value="productID" /><br> <s:if
							test="exchangeScore!=0">
							兑换积分：<s:property value="exchangeScore" />
							<br>
						</s:if> <s:if test="activityType.equals(\"t\")">
							最低团购人数：<s:property value="minGroupCount" />
							</br>
							团购价：<s:property value="tuanPrice" />
							</br>
						</s:if>
					</td>
					...
					<td nowrap="nowrap"><s:a
							href="activity!toEdit.action?e.id=%{id}">编辑</s:a></td>
				</tr>
			</s:iterator>
			<tr>
