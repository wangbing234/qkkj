...
<script type="text/javascript"
	src="<%=request.getContextPath() %>/resource/zTree3.1/js/jquery.ztree.all-3.1.min.js"></script>
...
<body style="text-align: center;">
	<s:form action="menu" name="form1" namespace="/manage" theme="simple">
		<div id="contians_div"
			style="text-align: right; border: 0px solid red; margin: auto;">
			<div id="context_div" style="margin-top: 20px;">

				<div id="showMsgDiv" style="text-align: center;"></div>
				<table id="result_table1" class="table table-bordered"
					style="width: 500px; margin: auto; margin-top: 20px;">
					<tr>
						<td colspan="2"
							style="background-color: #dff0d8; text-align: center;"><strong>当前选中的菜单</strong>
						</td>
					</tr>
					<tr style="display: none;">
						<td>id</td>
						<td><input id="input_menu_id" readonly="readonly"
							value='<s:property value="menu.id"/>' /></td>
					</tr>
					<tr style="display: none;">
						<td>pid</td>
						<td><input id="input_menu_pid" readonly="readonly"
							value='<s:property value="menu.pid"/>' /></td>
					</tr>
					<tr>
						<th>名称</th>
						<td style="text-align: left;"><input id="input_menu_name"
							size="60" value='<s:property value="menu.name"/>' /></td>
					</tr>
					<tr>
						<th>url</th>
						<td style="text-align: left;"><input id="input_menu_url"
							size="80" value='<s:property value="menu.url"/>' /></td>
					</tr>
					<tr>
						<th>类型</th>
						<td style="text-align: left;"><s:select
								list="#{'':'--请选择--','module':'模块','page':'页面','button':'按钮'}"
								id="input_menu_type" name="menu.type" cssClass="input-medium"
								listKey="key" listValue="value" /></td>
					</tr>
					<tr>
						<th>顺序</th>
						<td style="text-align: left;"><input id="input_menu_orderNum"
							value='<s:property value="menu.orderNum"/>' /> (菜单顺序从1开始，小的显示在前面)
						</td>
					</tr>
				</table>
				<!-- item -->

				<table id="result_table2" class="table table-bordered"
					style="width: 500px; margin: auto; margin-top: 20px;">
					<tr>
						<td colspan="2"
							style="background-color: #dff0d8; text-align: center;"><strong>添加菜单</strong>
							顶级模块<input type="radio" name="parentOrChildRadio" value="0" />
							顶级页面<input type="radio" name="parentOrChildRadio" value="1" />
							子模块<input type="radio" name="parentOrChildRadio" value="2" /> 子页面<input
							type="radio" name="parentOrChildRadio" value="3" /> 功能<input
							type="radio" name="parentOrChildRadio" value="4"
							checked="checked" /></td>
					</tr>
					<tr style="display: none;">
						<td>id</td>
						<td><input id="input_new_menu_id" readonly="readonly" /></td>
					</tr>
					<tr style="display: none;">
						<td>pid</td>
						<td><input id="input_new_menu_pid" readonly="readonly" /></td>
					</tr>
					<tr>
						<th>名称</th>
						<td style="text-align: left;"><input id="input_new_menu_name"
							size="60" /></td>
					</tr>
					<tr>
						<th>url</th>
						<td style="text-align: left;"><input id="input_new_menu_url"
							size="80" /></td>
					</tr>
					<tr>
						<th>类型</th>
						<td style="text-align: left;"><select
							id="input_new_menu_type">
					...
					<tr>
						...
						<td style="text-align: left;"><input
							id="input_new_menu_level" />(菜单顺序从1开始，小的显示在前面)</td>
					</tr>
					<tr>
						<td style="text-align: center;" colspan="2"><input
							type="button" id="add" value="修改或添加" class="btn btn-primary" /> <%-- 				<s:submit method="selectList" value="返回" cssClass="btn btn-inverse"/> --%>
						</td>
					</tr>
				</table>
			</div>
