...
<body>
	<s:form action="catalog" namespace="/manage" theme="simple" id="form"
		name="form">

		<input id="catalogID" value="<s:property value="e.pid"/>"
			style="display: none;" />
		<input id="catalogID_currentID" value="<s:property value="e.id"/>"
			style="display: none;" />
		...
		<table class="table table-bordered" style="width: 95%; margin: auto;">
			<tr style="background-color: #dff0d8">
				<td colspan="2"
					style="background-color: #dff0d8; text-align: center;"><strong>${isEdit==1?'编辑':'增加'}分类</strong>
			...
			<td style="text-align: left;">
				<!-- 							<input id="cc" class="easyui-combotree"  --> <%-- data-options="url:'<%=request.getContextPath() %>/manages/catalog/catalog!getRootWithTreegrid.action?e.type=<s:property value="e.type"/>',method:'get',required:true" style="width:200px;"> --%>
				<%
					...
					%>
				</s:if> <s:elseif test="e.type!=null and e.type.equals(\"a\")">
					<%
...
// 								}
							%>
				</s:else> <select onchange="catalogChange(this)" name="e.pid" id="catalogs" ${(isEdit==1 && e.pid==0)?'disabled':''}>
					<option value="">根分类</option>
					<s:iterator value="#request.catalogs" var="item">
			...
			<tr>
				<!-- <td style="text-align: right;">配图</td>
				<td style="text-align: left;" colspan="3"><input type="button"
					...
					value="我要上传" /> <s:textfield type="text" id="picture"
						name="e.picurl" ccc="imagesInput" />
					<div id="J_imageView">
			...
			<tr>
				...
				<td style="text-align: left;"><s:textfield name="e.name"
						id="name" data-rule="名称;required;name;" size="20" maxlength="20" /></td>
			</tr>
			<tr>
				<td style="text-align: right;">分类编码</td>
				<td style="text-align: left;">
					<!-- 							<input type="button" onclick="getCode()" value="自动获取" class="btn btn-default"/> -->
					<s:textfield name="e.code"
						data-rule="编码;required;code;length[1~45];remote[catalog!unique.action?e.id=${e.id}]"
						size="45" maxlength="45" id="code" />
				</td>
		...
		<!-- 	<tr>
				...
				<td style="text-align: left;"><s:textfield name="e.order1"
						data-rule="顺序;required;integer;order1;" size="20" maxlength="20"
						id="order1" /></td>
			</tr>
			...
			<s:if test="e.type!=null and e.type.equals(\"p\")">
				<tr>
					...
					<td style="text-align: left;"><s:select
							list="#{'n':'否','y':'是'}" id="showInNav" name="e.showInNav"
...
KindEditor.ready(function(K) {
	...
	var editor1 = K.editor({ allowFileManager : true });
						...
						editor1.plugin.multiImageDialog({
							clickFn : function(urlList) {
								...
								K.each(urlList, function(i, data) {
									...
									div.append('<img src="' + data.url + '" style="max-width: 50px;max-height: 50px;">');
								});
								editor1.hideDialog();
							}
						});
...
function selectDefaultCatalog(){
	...
	console.log("selectDefaultCatalog._catalogID="+_catalogID);
		$("#catalogs").val(_catalogID);
}

function catalogChange(obj){
	var selObj= $(obj).find("option:selected");
	var _pid = selObj.attr("pid");
	var _id = selObj.val();
