...
<body>
	<s:form action="cases" namespace="/manage" method="post" theme="simple">

		<table class="table table-bordered">
			<tr>
				<td colspan="4">
					<%-- 						<s:submit method="selectList" value="查询" cssClass="btn btn-primary"/> --%>
					<s:a method="selectList.action" cssClass="btn btn-primary">
						<i class="icon-search icon-white"></i> 查询
						</s:a> <%-- 						<s:submit method="toAdd" value="添加" cssClass="btn btn-success"/> --%>
					<a href="/manage/cases!toAdd.action" class="btn btn-success"> <i
						class="icon-plus-sign icon-white"></i> 添加
				</a> <%-- 						<s:submit method="deletes" onclick="return deleteSelect();" value="删除" cssClass="btn btn-danger"/> --%>
					<!-- 						<button method="cases!deletes.action" class="btn btn-danger" onclick="return submitIDs(this,'确定删除选择的记录?');"> -->
					<%-- 						<s:a method="deletes" cssClass="btn btn-danger" onclick="return deleteSelect();"> --%>
					<!-- 							<i class="icon-remove-sign icon-white"></i> 删除 --> <%-- 						</s:a> --%>
					<button method="cases!deletes.action" class="btn btn-danger"
			...
			<s:iterator value="pager.list">
				<tr>
					...
					<td>&nbsp; <img style="max-width: 100px; max-height: 100px;"
						alt=""
						src="<%=SystemManager.systemSetting.getImageRootPath() %><s:property value="picUrl"/>">
					...
					<td><s:a
							href="/manage/cases!toEdit.action?e.id=%{id}&e.type=%{e.type}">编辑</s:a>
					</td>
				</tr>
			</s:iterator>
			<tr>
