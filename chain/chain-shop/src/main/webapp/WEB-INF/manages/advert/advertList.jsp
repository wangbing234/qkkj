...
<body>
	<s:form action="advert" namespace="/manage" method="post"
		theme="simple">
		<table class="table table-bordered">
			<tr>
				...
				<td>类型</td>
				<td><s:select
						list="#{'pc_index_center':'PC首页中部','mobile_top':'手机端网页顶部','mobile_index_center':'手机端首页中部','mobile_index_bottom':'手机端首页底部','mobile_detail_top':'手机详细页顶部','mobile_detail_bottom':'手机详细页底部'}"
						id="style" name="e.code" cssClass="input-medium" headerKey=""
						headerValue="请选择" listKey="key" listValue="value" /></td>
				<!-- 
						<td>时间范围</td>
						<td>
			...
			<tr>
				<td colspan="16">
					...
					<%-- 								<s:param name="btnSelect" value="1">1</s:param> --%> <%-- 							</s:submit> --%>
					<%-- 							<s:a method="selectList" cssClass="btn btn-primary"> --%>
					<!-- 								<i class="icon-search icon-white"></i> 查询 --> <%-- 							</s:a> --%>
					<button method="advert!selectList.action" class="btn btn-primary"
					...
					<!-- 								<i class="icon-remove-sign icon-white"></i> 删除 --> <!-- 							</button> -->
		...
		<table class="table table-bordered table-hover">
			...
			<s:iterator value="pager.list">
				<tr>
					...
					<td class="aCss"><s:a href="advert!toEdit.action?e.id=%{id}">
							<s:property value="title" />
						</s:a></td>
					<td>&nbsp;<s:property value="code" /></td>
					<td>&nbsp;<s:property value="startdate" /> ~ <s:property
							value="enddate" /></td>
					...
					<td>&nbsp; <s:if test="useImagesRandom.equals(\"y\")">
							<img alt="显示"
								src="<%=request.getContextPath() %>/resource/images/action_check.gif">
						</s:if> <s:else>
							<img alt="不显示"
