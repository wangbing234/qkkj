...
<body>
	<s:form action="notifyTemplate!update.action" namespace="/manage"
		...
		<table class="table table-bordered">
			<tr style="background-color: #dff0d8">
				<td colspan="2"
					style="background-color: #dff0d8; text-align: center;"><strong>邮件、短信
						通知模板管理</strong></td>
			</tr>
			<tr>
				<td style="text-align: right; width: 80px;" nowrap="nowrap">选择模板</td>
				<td style="text-align: left;"><s:select
						list="notifyTemplateList" id="code" name="e.code"
						onchange="changeTemplate()" data-rule="模板:required;code;"
						headerKey="" headerValue="" listKey="code" listValue="name" />
			</tr>
			<tr>
				<td style="text-align: right;" nowrap="nowrap">参数解释</td>
				<td style="text-align: left;">
					<div id="remarkDiv">
						<s:property value="e.remark" />
					</div>
				</td>
			</tr>
			<tr>
				<td style="text-align: right;" nowrap="nowrap">模板内容</td>
				<td style="text-align: left;"><s:textarea name="e.template"
						id="template" style="width:100%;height:400px;visibility:hidden;"
						data-rule="模板内容:required;template;"></s:textarea>
			</tr>
			<tr>
				<td colspan="28" style="text-align: center;">
					<!-- 					<input type="button" onclick="updateTemplate()" value="保存" class="btn btn-primary"/> -->
					<%-- 					<s:submit value="保存" cssClass="btn btn-primary"/> --%> <%-- 					<s:a cssClass="btn btn-success"> --%>
					<!-- 						<i class="icon-ok icon-white"></i> 保存 --> <%-- 					</s:a> --%>
					<button method="notifyTemplate!update.action"
	...
	function changeTemplate(){
		...
		var _url = "/manage/notifyTemplate!selectTemplateByCode.action?e.code="+_code;
	...
	function updateTemplate(){
		...
		var _url = "/manage/notifyTemplate!updateTemplate.action";
		console.log("_url="+_url);
		$.ajax({
		  type: 'POST',
		  url: _url,
