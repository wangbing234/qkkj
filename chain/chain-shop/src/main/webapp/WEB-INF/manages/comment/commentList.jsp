...
<body>
	<s:form action="comment" namespace="/manage" method="post"
		theme="simple">
		<table class="table table-bordered">
			<tr>
				...
				<td style="text-align: left;"><s:textfield name="e.orderID"
						cssClass="input-medium" /></td>

				<td style="text-align: right;">是否显示</td>
				<td style="text-align: left;"><s:select
						list="#{'':'','y':'是','n':'否'}" id="status" name="e.status"
						cssClass="input-medium" listKey="key" listValue="value" /></td>
			</tr>
			<tr>
				<td colspan="16">
					<%-- 							<s:submit method="selectList" value="查询" cssClass="btn btn-primary"> --%>
					<!-- 								<i class="icon-search"></i> --> <%-- 							</s:submit> --%>
					<%-- 							<s:a method="selectList" cssClass="btn btn-primary"> --%>
					<!-- 								<i class="icon-search icon-white"></i> 查询 --> <%-- 							</s:a> --%>
					<button method="comment!selectList.action" class="btn btn-primary"
						onclick="selectList(this)">
						<i class="icon-search icon-white"></i> 查询
					</button> <a href="/manage/comment!addInit.action" class="btn btn-success">
						<i class="icon-plus-sign icon-white"></i> 添加
				</a>
					<button method="comment!updateStatusY.action"
						class="btn btn-warning"
						onclick="return submitIDs(this,'确定显示指定的记录吗？');">
						<i class="icon-arrow-up icon-white"></i> 显示
					</button>
					<button method="comment!updateStatusN.action"
						class="btn btn-warning"
						onclick="return submitIDs(this,'确定不显示指定的记录吗？');">
						<i class="icon-arrow-down icon-white"></i> 不显示
					</button>

					<div
						style="float: right; vertical-align: middle; bottom: 0px; top: 10px;">
						<%@ include file="/WEB-INF/manages/system/commontPager.jsp"%>
					</div>
				</td>
			</tr>
		</table>
			...
			<s:iterator value="pager.list">
				<tr>
					...
					<td>
						...
						<a target="_blank"
						href="<%=SystemManager.systemSetting.getWww()%>/product/<s:property value="productID" />.html"><s:property
								value="productID" /></a>
					</td>
					<td><s:if test="status.equals(\"p\")">
					系统（<s:property value="nickname" />）
							</s:if>
						<s:else>

							<s:a target="_blank"
								href="accounts!show.action?account=%{account}">
								<s:property value="account" />
							</s:a>
						</s:else></td>
					<td>&nbsp; <a target="_blank"
						href="order!toEdit.action?e.type=show&e.id=<s:property value="orderID" />"><s:property
								value="orderID" /></a>
					</td>
					<td width="500px">&nbsp; <s:property value="content" /><br>

						<s:if test="reply!=null and reply!=''">
							<span style="color: #f50">【已回复】</span>：<s:property value="reply"
								  />
						</s:if>
					</td>
					...
					<td>&nbsp; <s:if
							test="status.equals(\"y\") or status.equals(\"p\")">
							<img alt="显示"
					...
					<td nowrap="nowrap"><s:if test="status.equals(\"p\")">

						</s:if> <s:elseif test="reply==null">
							<s:a href="comment!toEdit.action?e.id=%{id}">回复</s:a>
						</s:elseif> <s:else>
							<s:a href="comment!toEdit.action?e.id=%{id}&update=y">修改回复</s:a>

						</s:else> <s:a href="comment!delete.action?e.id=%{id}">删除</s:a></td>
				</tr>
			</s:iterator>

			<tr>
				<td colspan="17" style="text-align: center;"><%@ include
						file="/WEB-INF/manages/system/commontPager.jsp"%></td>
			</tr>
		</table>
	</s:form>
</body>
