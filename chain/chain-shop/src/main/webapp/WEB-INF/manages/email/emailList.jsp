...
<body>
	<s:form action="email!selectList.action" namespace="/manage"
		...
		<table class="table table-bordered">
			<tr>
				...
				<td style="text-align: left;"><s:select
						list="#{'':'','y':'发送成功','n':'发送失败'}" id="sendStatus"
						name="e.sendStatus" cssClass="input-medium" listKey="key"
						listValue="value" /></td>
			</tr>
			<tr>
				<td colspan="28">
					...
					<button method="email!selectList.action" class="btn btn-primary"
			...
			<s:iterator value="pager.list">
				<tr>
					...
					<td nowrap="nowrap">&nbsp; <s:property
							value="notifyTemplateName" />
					</td>
					<td nowrap="nowrap">&nbsp;<s:property value="createdate" /></td>
					<td nowrap="nowrap">&nbsp; <s:if
							test="sendStatus.equals(\"y\")">
							<img alt="发送成功"
								src="<%=request.getContextPath() %>/resource/images/action_check.gif">发送成功
						</s:if> <s:elseif test="sendStatus.equals(\"n\")">
							<img alt="发送失败"
								src="<%=request.getContextPath() %>/resource/images/action_delete.gif">发送失败
						</s:elseif> <s:else>
							发送中...
						</s:else>
					</td>
					<td nowrap="nowrap">&nbsp; <s:if test="status.equals(\"y\")">
							<img alt="已失效"
								src="<%=request.getContextPath() %>/resource/images/action_delete.gif">已失效
						</s:if> <s:else>
							<img alt="未失效"
								src="<%=request.getContextPath() %>/resource/images/action_check.gif">未失效
						</s:else>
					</td>
